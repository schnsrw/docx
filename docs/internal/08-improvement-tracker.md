...
| 🟠  | **P3 #40 (1/3)** — `agent/context.ts:193` `inTable` TODO replaced with a design note. The flag was misleadingly marked as a stub; `paragraphs` filters `body.content` to top-level blocks only, so `inTable = false` is correct for the current addressing scheme. Other two TODOs (footnote-style cascade, M2 host seed) are documented future work — left in place. | `agent/context.ts`                                                                               |
